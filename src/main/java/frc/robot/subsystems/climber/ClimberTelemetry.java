...
public class ClimberTelemetry {
	...
	public void log() {
		...
		// Named position
		DogLog.log("Climber/Position", getNamedPosition(climber.goalState.position));
	...
	private String getNamedPosition(double goalMeters) {
		double tol = 0.01;
		if (Math.abs(goalMeters - MIN_HEIGHT.in(Meters)) < tol) return "Bottom";
		if (Math.abs(goalMeters - MAX_HEIGHT.in(Meters)) < tol) return "Top";
		if (Math.abs(goalMeters - HANG_HEIGHT.in(Meters)) < tol) return "Hang";
		if (Math.abs(goalMeters - RELEASE_HEIGHT.in(Meters)) < tol) return "Release";
		return "Custom";
	}
}
