...
public class SwerveSubsystem extends SubsystemBase {
	...
	private final TrapezoidProfile.Constraints constraints = new TrapezoidProfile.Constraints(
			Constants.MAX_ANGULAR_SPEED / 2, Constants.MAX_ANGULAR_ACCELERATION / 2);
	// private final ProfiledPIDController profiledController =
	// new ProfiledPIDController(0.5, 0.1, 0.05, constraints);
	private final TrapezoidProfile headingProfile = new TrapezoidProfile(constraints);
	private final PIDController headingController = new PIDController(0.8, 0.2, 0);
	/**
	...
	double speed = Math.PI * 1.5;
	double midSpeed = 2;
	double lowSpeed = 0.6;
	double lowestSpeed = 0.3;
	...
	public Command aimAt(DoubleSupplier translateX, DoubleSupplier translateY, Pose2d target) {
		return startRun(() -> {
			...
			// profiledController.setPID(aimKp.get(), aimKi.get(), aimKd.get());
			// profiledController.reset(getPose().getRotation().getRadians(),
			...
			double turnToAngle = desiredAngle + currentPose.getRotation().getRadians();
			double difAngle = currentPose.getRotation().getRadians() - desiredAngle;
			double amt = 0.0;
			if (Math.abs(difAngle) < tolerance) {

			} else if (difAngle > 0) {
				if (Math.abs(difAngle) > Math.PI / 2) {
					amt = -speed;
				} else if (Math.abs(difAngle) > Math.PI / 8) {
					amt = -midSpeed;
				} else if (Math.abs(difAngle) > Math.PI / 12) {
					amt = -lowSpeed;
				} else {
					amt = -lowestSpeed;
				}
				// this.drive(new Translation2d(0.0, 0.0), difAngle, true);
			} else if (difAngle < 0) {
				if (Math.abs(difAngle) > Math.PI / 2) {
					amt = speed;
				} else if (Math.abs(difAngle) > Math.PI / 8) {
			...
			speeds.omegaRadiansPerSecond = amt;

			drive(speeds);

			DogLog.log("PID/difAngle", difAngle, Radians);
			DogLog.log("PID/desiredAngle", desiredAngle, Radians);
			DogLog.log("PID/currentAngle", currentPose.getRotation().getRadians(), Radians);
			DogLog.log("logz", logz);
	 ...
	 *
	 * @param translation
	 *          {@link Translation2d} that is the commanded linear velocity of the robot, in
	 ...
	 * @param rotation
	 *          Robot angular rate, in radians per second. CCW positive. Unaffected by
